...
pub fn start_out_connect_loop(
    ...
    let (conn_send, conn_recv) = channel::unbounded();
...
fn start_connect_loop<D: MidiIO, C>(
    fuzzy_port_name: String,
    mut driver_factory: impl FnMut() -> Result<D, InitError> + Send + 'static,
    mut connect: impl FnMut(D, &D::Port, &str) -> Result<C, ConnectError<D>> + Send + 'static,
