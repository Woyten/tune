...
use crate::{
    fluid::{FluidControl, FluidMessage},
    keypress::{IllegalState, KeypressTracker, LiftAction, PlaceAction},
    magnetron::{envelope::EnvelopeType, waveform::WaveformSpec},
    model::{EventId, EventPhase},
    synth::{SynthControl, WaveformControl},
};
...
struct PianoEngineModel {
    ...
    waveform_control: WaveformControl<EventId>,
    fluid_control: FluidControl,
...
impl PianoEngine {
    ...
    pub fn control(&self, control: SynthControl, value: f64) {
        self.lock_model().waveform_control.control(control, value);
    }
...
impl PianoEngineModel {
        ...
        let fluid_message = match message_type {
            ...
            ChannelMessageType::PolyphonicKeyPressure { key, pressure } => {
                self.waveform_control
                    .update_pressure(EventId::Midi(key), f64::from(pressure) / 127.0);
            ...
            ChannelMessageType::ControlChange { controller, value } => {
                ...
                if controller == self.cc_numbers.modulation {
                    self.waveform_control
                        .control(SynthControl::Modulation, value);
                }
                if controller == self.cc_numbers.breath {
                    self.waveform_control.control(SynthControl::Breath, value);
                }
                if controller == self.cc_numbers.foot {
                    self.waveform_control.control(SynthControl::Foot, value);
                }
                if controller == self.cc_numbers.expression {
                    self.waveform_control
                        .control(SynthControl::Expression, value);
                }
                if controller == self.cc_numbers.damper {
                    self.waveform_control.damper(value);
                    self.waveform_control.control(SynthControl::Damper, value);
                }
                if controller == self.cc_numbers.sostenuto {
                    self.waveform_control
                        .control(SynthControl::Sostenuto, value);
                }
                if controller == self.cc_numbers.soft {
                    self.waveform_control
                        .control(SynthControl::SoftPedal, value);
                }
            ...
            ChannelMessageType::ChannelPressure { pressure } => {
                self.waveform_control
                    .control(SynthControl::ChannelPressure, f64::from(pressure) / 127.0);
                FluidMessage::Monophonic(message_type)
            }
            // Forwarded to all channels of all synths.
        ...
        match phase {
            EventPhase::Pressed(velocity) => match self.synth_mode() {
                SynthMode::Waveform {
                    ...
                    self.waveform_control.start(id, waveform);
            ...
            EventPhase::Moved => {
                if self.legato {
                    self.waveform_control.update_pitch(id, pitch);
    ...
    fn set_program(&mut self, program: u8) {
        match self.synth_mode_mut() {
            ...
            SynthMode::Fluid { .. } => {
                self.fluid_control.send(FluidMessage::Monophonic(
    ...
    fn retune(&mut self) {
        ...
        self.fluid_control.send(FluidMessage::Retune {
    ...
    fn send_note_on(&mut self, location: KeyLocation, velocity: u8) {
        match location {
            KeyLocation::FluidSynth((channel, note)) => {
                self.fluid_control.send(FluidMessage::Polyphonic(
